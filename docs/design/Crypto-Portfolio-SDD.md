...
Версия 1.9 · 20 сентября 2026 · Go, DDD/Clean Architecture и журнал проверки осуществимости.

**Статус:** Go-диагностика опубликована в публичном репозитории `DanilaKorobkov/crypto-portfolio`, ветка `develop`, commit `77f1edb18448b59ab624b1577538d3d18bc6cf1d`. GitHub write-доступ восстановлен после установки приложения пользователем. Локальные проверки и GitHub Actions offline smoke успешно завершились: форматирование, `go vet`, 24 Go-теста с race detector и CLI без конфигурации. Добавлено age-шифрование checkpoints и запрет настроенного сбора в Actions без получателя; 31 тест прошёл локально и в новом GitHub Actions CI. Доставка зашифрованного отчёта и live-сбор пока не настроены. Реальных данных кошельков нет. Авторизация GitHub CLI остановлена сетевой политикой runtime; повторный вход до изменения доступа не требуется. Последнее состояние — §14.15, результаты исследования — §14.9. Это не готовый отчёт о портфеле.
...
**Этап 0 — доказать получение данных и канал исполнения.** Проверить бесплатный runtime и доступ из чата; подключить бесплатные ключи при необходимости; обнаружить и сверить позиции двух адресов; заполнить матрицу сетей/протоколов; выполнить текущие и исторические чтения; проверить бесплатное долговечное хранение. Выход: воспроизводимый JSON фактических позиций, список пробелов и выбранный исполнитель. Сейчас этап не пройден.

**Этап 1 — текущий портфель.** Адаптеры обнаруженных протоколов, оценка, долг, нормализация, HF/ликвидация и LP-диапазоны. Выход: первый отчёт с подтверждёнными текущими показателями и маркировкой недоступного.

**Этап 2 — доход.** Комиссии/rewards, vault accounting, исторические окна и средняя стоимость. Выход: проверенный измеренный APR там, где хватает данных; отсутствие показателя там, где восстановить историю нельзя бесплатно.

**Этап 3 — устойчивость и завершение v1.** Дедлайн, retries, восстановление состояния, новая позиция, partial reports, тест нулевых расходов и end-to-end из чата. Выход: работающий сценарий по запросу в пределах проверенного покрытия.

После v1 можно отдельно спроектировать утренние отчёты и события HF/LP. Они не включаются скрыто в текущую реализацию.
